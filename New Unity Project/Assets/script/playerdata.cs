...
public class playerdata : MonoBehaviour {
    public struct shuxing
    {
        ...
        public int PER;//感知
        public int PSY;//精神
        public int WIL;//意志HUIGEN
        public int CHA;//魅力 隐藏
    ...
    public static roledata roledatainfo = roledata_init(roledatainfo);
...
// Update is called once per frame
    ...
    public static roledata roledata_init(roledata roledatainfo)  
    {
        ...
        shuxing_init(roledatainfo.shuxing_basis);
        roledatainfo.shuxing_cur=roledatainfo.shuxing_basis;
        skill_work_init(roledatainfo.skill_work_basis);
        roledatainfo.skill_work_cur = roledatainfo.skill_work_basis;
        skill_culture_init(roledatainfo.skill_culture_basis);
        roledatainfo.skill_culture_cur = roledatainfo.skill_culture_basis;
        skill_fight_init(roledatainfo.skill_fight_basis);
        roledatainfo.skill_fight_cur = roledatainfo.skill_fight_basis;
        return roledatainfo;
    }
    public static void shuxing_init(shuxing roledata)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            attributie_basis[i] = GetNum(2, 1, 6);
        }
        //随机隐藏属性
        for (int i = 0; i < 5; i++)
        {
            attributie_hide[i] = GetNum(2, 0, 3);
        }
            ...
            roledata.CHA = attributie_hide[0];//魅力
            roledata.FUYUAN = attributie_hide[1]; //福缘
            roledata.HUIGEN = attributie_hide[2]; //慧根
            roledata.GENGU = attributie_hide[3]; //根骨
    }
    public static void skill_work_init(skill_work roledata)
    {
        int[] attributie_basis = new int[4];
        //随机基础属性
        for (int i = 0; i < 4; i++)
        {
            attributie_basis[i] = GetNum(2, 1, 10);
        }
        //随机隐藏属性
    ...
    public static void skill_culture_init(skill_culture roledata)
    {
    ...
    public static void skill_fight_init(skill_fight roledata)
    {
    ...
    public static int GetNum(int num, int minValue, int maxValue)
    {
        ...
        while (n < num)
        {
            ...
            randnum = ra.Next(minValue-1, maxValue+1);
            tmp = tmp + randnum;
            n = n + 1;
        }
        return tmp;
